...
This is a repost of [my post on the CloudBees Developers blog](http://developer-blog.cloudbees.com/2012/10/the-cross-build-injection-attack-fallacy.html)
...
Take for example [this attack vector using Maven as an example victim build tool](http://branchandbound.net/blog/security/2012/10/cross-build-injection-in-action/). How does the attack work? Well it replaces a good artifact in the Maven local repository with a bad version… and bad things happen.
...
What is the solution? Simple. Don't do the checks until you are making the release build! Better yet do the release builds from a continuous integration server such as [Jenkins](http://www.jenkins-ci.org/). You can lock that down, have it do the checks for you, and have it sign the resultant artifacts… but just be sure that you trust its filesystem and your source control system too!