---
title: "Writing a Hudson plugin (Part 5½ - Typos corrected)"
...
I left a "`css`" attribute in the `<l:layout>` tag.
...
The `<j:if>` should be based on the `from` variable and not `it`
...
The `<j:if>` had an extra `}` in the expression
