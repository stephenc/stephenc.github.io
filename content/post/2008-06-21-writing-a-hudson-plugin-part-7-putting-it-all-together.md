...
This report is only used for the Maven2 project types. The two key methods are `update` which is called as each module completes, and `getProjectAction` which should return the project level aggregated report _if there is a report to show_. At this point we're ready for the individual project report:
